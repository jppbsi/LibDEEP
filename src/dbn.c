...
double BernoulliDBNTrainingbyContrastiveDivergencewithDropout(Dataset *D, DBN *d, int n_epochs, int n_CD_iterations, int batch_size, gsl_vector *p, gsl_vector *q){
    double error = 0.0, aux = 0.0;
    Dataset *tmp1 = NULL, *tmp2 = NULL;
    int i, j, z, id;
    ...
    for(id = 0; id < d->n_layers; id++){
	    fprintf(stderr,"\nTraining layer %i ... ", id+1);
	    error = BernoulliRBMTrainingbyContrastiveDivergencewithDropout(tmp1, d->m[id], n_epochs, n_CD_iterations, batch_size, gsl_vector_get(p, id), gsl_vector_get(q, id));
...
double BernoulliDBNTrainingbyPersistentContrastiveDivergencewithDropout(Dataset *D, DBN *d, int n_epochs, int n_CD_iterations, int batch_size, gsl_vector *p, gsl_vector *q){
    double error, aux;
    Dataset *tmp1 = NULL, *tmp2 = NULL;
    int i, j, z, id;
    ...
    for(id = 0; id < d->n_layers; id++){
        ...
        fprintf(stderr,"\nTraining layer %d ... ", id+1);
        error = BernoulliRBMTrainingbyPersistentContrastiveDivergencewithDropout(tmp1, d->m[id], n_epochs, n_CD_iterations, batch_size, gsl_vector_get(p, id), gsl_vector_get(q, id));
...
double BernoulliDBNTrainingbyFastPersistentContrastiveDivergencewithDropout(Dataset *D, DBN *d, int n_epochs, int n_CD_iterations, int batch_size, gsl_vector *p, gsl_vector *q){
    double error, aux;
    Dataset *tmp1 = NULL, *tmp2 = NULL;
    int i, j, z, id;
    ...
    for(id = 0; id < d->n_layers; id++){
        fprintf(stderr,"\nTraining layer %d ... ", id+1);
        error = BernoulliRBMTrainingbyFastPersistentContrastiveDivergencewithDropout(tmp1, d->m[id], n_epochs, n_CD_iterations, batch_size, gsl_vector_get(p, id), gsl_vector_get(q, id));
...
/* It reconstructs an input dataset given a trained DBN */
double BernoulliDBNReconstructionwithDropout(Dataset *D, DBN *d, gsl_vector *p, gsl_vector *q){
    gsl_vector *h_prime = NULL, *v_prime = NULL, *aux = NULL;
    double error = 0.0;
    int l, i;
    
    for (l = 0; l < d->n_layers; l++)
	gsl_matrix_scale(d->m[l]->W, gsl_vector_get(p, l)*gsl_vector_get(q, l));
