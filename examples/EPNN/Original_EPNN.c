...
int main(int argc, char **argv)
{
    ...
    else if ((j == argc - 2) && (learningPhase == 0))
    ...
    /* Loading datasets */
    fflush(stderr);
    fprintf(stdout, "\nReading training set [%s] ...", argv[train_set]);
    fflush(stdout);
    ...
    if (learningPhase)
    {
        ...
        fprintf(stdout, "\nReading evaluating set [%s] ...", argv[eval_set]);
        fflush(stdout);
        ...
        if (learningPhase == 2)
        {
            fprintf(stdout, "\nMerge Training set and Evaluating set to Training Phase");
            fflush(stdout);
    ...
    if (kmax)
    {
        fprintf(stdout, "\n\nClustering [%s] with kmax: %i... ", argv[train_set], kmax);
        fflush(stdout);
        gettimeofday(&tic, NULL);
    ...
    fprintf(stdout, "\nAllocating training set [%s] ...", argv[train_set]);
    fflush(stdout);
    if (radius)
    ...
    time += (((toc.tv_sec - tic.tv_sec) * 1000.0 + (toc.tv_usec - tic.tv_usec) * 0.001) / 1000.0);
    fprintf(stdout, "\nAllocating training time: %f seconds\n", (((toc.tv_sec - tic.tv_sec) * 1000.0 + (toc.tv_usec - tic.tv_usec) * 0.001) / 1000.0));
    fflush(stdout);
    ...
    /* Testing phase */
    fprintf(stdout, "\nReading testing set [%s] ...", argv[test_set]);
    fflush(stdout);
