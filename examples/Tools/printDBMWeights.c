...
int main(int argc, char **argv){
    ...
    if(argc != 8){
        fprintf(stderr,"\nusage DBM <training set> <input parameters file> <model file> <number of DBM layers> <output file name> <number of images> <layer #>\n");
        exit(-1);
    }
    int i, j;
    ...
    loadDBMParametersFromFile(d,argv[2]);
